...
public class LibraryController {
    ...
    public String register(@RequestParam String username, @RequestParam String password,
            @RequestParam String repassword, @RequestParam String gender, @RequestParam String number,
            Model model) {
        // 获得所有参数
        ...
        if (password.equals(repassword) == false) {
            ...
            if ("success".equals(registerResult)) {
                ...
                model.addAttribute("fail", "注册失败，该用户名已经存在");
                return "register";
            }
        }
    ...
    public String modifyUserEdit(@RequestParam String username, @RequestParam String password,
            @RequestParam String repassword, @RequestParam String authority, @RequestParam String gender,
    ...
    public String queryUser(@RequestParam String showPage, @RequestParam String keyWord, Model model,
        ...
        long recordAmount = userService.queryRecordAmount(keyWord);
        pageInfo.setPageNum(recordAmount);
        pageInfo.showPageCheck();

        List<User> list = userService.queryUser(pageInfo, keyWord);
    ...
    /*
     * 所属功能：个人信息修改 实现：向个人信息修改的编辑页面提供原先未修改的原始值
     */
    @RequestMapping(value = "updatePrivateInfo")
    public String updatePrivateInfo(@RequestParam String keyWord, Model model)
    ...
    /*
     * 所属功能:修改个人信息(普通用户修改) 实现:将经过修改后的个人信息写进数据库里面,覆盖原来的信息 标签：写
     */
    @RequestMapping(value = "updatePrivateInfo", method = RequestMethod.POST)
    public String updatePrivateInfo(@RequestParam String username, @RequestParam String password,
            @RequestParam String repassword, @RequestParam String gender, @RequestParam String number,
            Model model) {
    ...
    public String modifyBook(@RequestParam String bookid, @RequestParam String bookname,
            @RequestParam String authod, @RequestParam String publishCompany,
            @RequestParam String publishTime, @RequestParam String totalAmount, Model model) {
    ...
    public String query(@RequestParam String keyWord, @RequestParam String showPage, Model model,
            HttpServletRequest request) throws UnsupportedEncodingException {
        Page pageInfo = new Page();
        pageInfo.setShowPage(showPage);// 设置要显示的页数
        pageInfo.setPageRecord(2);// 设置每一页的记录数

        long recordAmount = bookService.queryRecordAmount(keyWord);// 获得总的记录数
        pageInfo.setPageNum(recordAmount);
        pageInfo.showPageCheck();// 对错误的showPage进行异常处理
        List<Book> list = bookService.queryBook(pageInfo, keyWord);
